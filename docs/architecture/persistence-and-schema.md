...
| `STORAGE_KEY` | `` `artworkChecklist:v${CURRENT_SCHEMA_VERSION}` `` → `"artworkChecklist:v5"` |
| `LEGACY_STORAGE_KEYS` | `["artworkChecklist:v4", "artworkChecklist:v3", "artworkChecklist:v2", "artworkChecklist:v1"]` |
...
| v5 | Cross-functional reviewer snapshots, three required department sign-offs, optional per-department PNG signatures and revision binding | `migrateStateV4ToV5` + `addCrossFunctionalSignOffs` |

This history is supported by code and tests: v1/v2/v3/v4 fixtures are migrated and asserted in `d-persistence.test.js`, `g4-multi-layer-artwork.test.js`, `g5-pantone-compliance.test.js` and `h-cross-functional-signoff.test.js`.
...
- `migrateState()`: current version returned unchanged; compatible versions traverse the v1→v2→v3→v4→v5 chain; unsupported version → `console.warn` + `null`.
- `validateState()`: structural workspace validation (see [data-model.md](data-model.md)).
- `rehydrateState()` → `rehydrateProduct` → `rehydrateItems` / `rehydrateSignOffs`: builds a **fresh object graph** (no references into parsed JSON), restores canonical items/departments and clones pins, reviewer snapshots and signatures.
...
| `addCrossFunctionalSignOffs(productLike)` | Assigns three canonical Pending sign-offs (never trusts a legacy-version sign-off field); shared by workspace/import migrations |
| `migrateStateV4ToV5(state)` | Deep-clones; adds sign-offs to every product; `schemaVersion=5` |
...
1. `validateState` requires **exactly** `schemaVersion === 5` — v1/v2/v3/v4 in-memory states are invalid and must be migrated first.
2. Imports of v1/v2/v3/v4 files are migrated by `migrateImportData` before validation (`validateImportData`).
