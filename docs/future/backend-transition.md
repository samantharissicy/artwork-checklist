...
> This document maps what **would change** if a backend became a real requirement (planned M). It is not an implementation plan and no backend exists today.

## Motivation (when this becomes relevant)

Real needs only — per the development plan: multiple users, multiple computers, shared history, organizational approval, audit, permissions, central artwork storage.
...
- [layer-planning.md](layer-planning.md)
