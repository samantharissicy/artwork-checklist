...
| Cross-functional approvals | Department sign-off workflow (roadmap H) |
| Reporting needs | Report model + print/PDF (roadmap J) |
...
This shape is **illustrative only**: concrete technologies (framework, language, database vendor) are deliberately not chosen until the roadmap decides them.
...
| --- | --- |
| `js/app.js` is one classic script (~9,800 lines) | Module split (roadmap L1) should precede large future features |
| State discipline is by convention | A backend client layer must respect the same mutation/validation boundaries |
| No report model | Roadmap J must introduce `buildReportData(product)` before any print/PDF |
| No revision model | Roadmap M3 must introduce Product → Artwork → Revision before audit history |

## Related Documents

- [backend-transition.md](backend-transition.md) — the concrete migration map if a backend appears.
- [roadmap-technical-notes.md](roadmap-technical-notes.md) — layer dependencies.
