...
Deeply document the migration strategy: philosophy, the full chain v1→v2→v3→v4, legacy storage keys, and the tests that protect migrations.
...
- v3 file → `addPantoneComplianceItem` → v4.
- Unsupported/invalid → `null` → import rejected.

Note: import payloads have no `products` wrapper, so `migrateStateV3ToV4` cannot be applied to them directly — the import branch uses `addPantoneComplianceItem` on the items collection (this is why the helper exists: it is shared by both paths).
...
| Round-trip stability | `G5R-049`, B1 serialization tests |
